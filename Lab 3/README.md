...
This job started 20 trainigs jobs in 2 batches (10 jobs per batch). All training jobs within batch will run in paralell. The maximum numebr of 20 training jobs was configured by use when we launch the Autopilot job by calling the `create_auto_ml_job` API. 10 training per batch it is a default configuration:
